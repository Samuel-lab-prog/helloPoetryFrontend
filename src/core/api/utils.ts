...
export function createQueryKeys<T extends Record<string, (...args: any[]) => readonly unknown[]>>(keys: T) {
  return keys;
}
...
export function createQueryEndpoint<TArgs extends unknown[], TResult>(
  ...
  return {
    ...
    fetch: (
      ...args: [...TArgs, FetchQueryOptions<TResult>?]
    ) => {
    ...
    prefetch: (...args: TArgs) =>
      queryClient.prefetchQuery(query(...args)),
    ...
    getCache: (...args: TArgs) =>
      queryClient.getQueryData<TResult>(config.key(...args)),
    ...
    optimisticUpdate(
      args: TArgs,
      updater: (current: TResult | undefined) => TResult
    ) {
