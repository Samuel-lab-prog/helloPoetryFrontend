...
test.describe('logged out immersive poem page', () => {
	...
	test('shows an unavailable card without keeping the previously loaded immersive poem', async ({
		...
		await page.goto(`/poems/${restrictedPoemRef.slug}/${restrictedPoemRef.id}/immersive`);
	...
	test('shows a not found card in immersive mode', async ({ page }) => {
		const errorMock = await mockPoemErrorPage(page, {
			poemId: missingPoemRef.id,
			status: 404,
		});

		await page.goto(`/poems/${missingPoemRef.slug}/${missingPoemRef.id}/immersive`);
	...
	test('does not request the API when the immersive poem id is invalid', async ({ page }) => {
		...
		await page.goto('/poems/not-a-number/immersive');
